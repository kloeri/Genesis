...
void
command_line::add_argument(argument *argument)
{
...
const argument *
command_line::get_argument(const std::string & long_name) const
{
    std::map<std::string, argument *>::const_iterator argument(_long_names.find(long_name));
...
void
command_line::parse(const int argc, const char * const * const argv)
{
    ...
    for (argument = arguments.begin(); argument != arguments.end(); ++argument)
    {
        ...
        {
            parameter.erase(0, 2);
            std::map<std::string, genesis::argument *>::const_iterator it(_long_names.find(parameter));
        ...
        {
            parameter.erase(0, 1);
            for (std::string::const_iterator p(parameter.begin()); p != parameter.end(); p++)
            {
                std::map<char, genesis::argument *>::const_iterator it(_short_names.find(*p));
...
command_line::parameters(void) const
{
    return _parameters;
}

std::ostream &
genesis::operator<<(std::ostream & os, const command_line & command_line)
{
    std::list<genesis::argument *>::const_iterator argument;

    for (argument = command_line._arguments.begin(); argument != command_line._arguments.end(); ++argument)
    {
