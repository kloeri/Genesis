...
int GenerateEventsCallback(const char *name, const struct stat *filestat,
                           int flags, struct FTW *ftwbuf) {
  std::string eventname(name);

  if (filestat->st_mode & S_IRUSR) {
    if ((eventname.length() >= 7) &&
        (eventname.compare(eventname.length() - 7, 7, "/uevent") == 0)) {
      ...
      // We don't care about /sys but coldplug events should start with add@ to
      // signify adding new devices
...
void NetlinkUevent::GenerateEvents() {
  if ((UEventConfiguration->get_option("coldplug") == "yes") &&
      (UEventConfiguration->get_option("coldplug_mounts_sysfs") == "yes")) {
    if (mount("none", "/sys", "sysfs", MS_RDONLY | MS_NOEXEC, NULL) == -1) {
      std::perror("mount");
    }
  }

  nftw("/sys/class/", GenerateEventsCallback, 1, FTW_ACTIONRETVAL);
...
NetlinkUevent::NetlinkUevent() {
  ...
  UEventConfiguration =
      new Configuration(SYSCONFDIR "config", "netlink-uevent", defaultconfig);
  SourceScripts(SYSCONFDIR "netlink-uevent/");

  if (UEventConfiguration->get_option("coldplug") == "yes") {
    Log::get_instance().log(DEBUG, NETLINK_UEVENT,
                            "Generating coldplug events..");
    GenerateEvents();
  }
...
void NetlinkUevent::SourceScripts(std::string path) {
  ...
  if (n >= 0) {
      ...
      while (envvars.good()) {
        ...
        if (line != "") {
          ...
          Log::get_instance().log(
              DEBUG, NETLINK_UEVENT,
              "NetlinkUevent::SourceScripts adding event: " + match);
    ...
    Log::get_instance().log(ERR, NETLINK_UEVENT,
...
// NETLINK_ROUTE
//
void *NetlinkUevent::OpenSocket(int domain, int type, int protocol,
                                int multicastgroup) {
...
NetlinkUevent::~NetlinkUevent() { close(netlinksocket); }
...
Action *NetlinkUevent::ProcessEvent(std::string event) {
  ...
  if (event.find("remove@") != std::string::npos)
    event.replace(event.find("remove@"), 7,
                  "G_EVENTTYPE=removedevice;G_DEVICE=");
  if (event.find("change@") != std::string::npos)
    event.replace(event.find("change@"), 7,
                  "G_EVENTTYPE=changedevice;G_DEVICE=");
...
int NetlinkUevent::get_fd() { return netlinksocket; }
