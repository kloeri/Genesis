...
GenesisPipe::GenesisPipe(void)
{
    if (::mkfifo(GenesisPipe::Name.c_str(), 0666) != 0)
    {
        Log::get_instance().log(ERR, GENESIS_PIPE,
                                "Unable to create pipe: " + std::string(::strerror(errno)));
    ...
    {
        Log::get_instance().log(ERR, GENESIS_PIPE,
                                "Unable to chmod pipe: " + std::string(::strerror(errno)));
        return; // XXX Should this throw an exception?
    }
    ...
    {
        Log::get_instance().log(ERR, GENESIS_PIPE,
                                "Unable to open pipe: " + std::string(::strerror(errno)));
        return; // XXX Should this throw an exception?
    }
}
...
GenesisPipe::~GenesisPipe(void)
{
    ...
    {
        Log::get_instance().log(ERR, GENESIS_PIPE,
                                "Unable to close pipe: " + std::string(::strerror(errno)));
    }

    if (::unlink(GenesisPipe::Name.c_str()) != 0)
    {
        Log::get_instance().log(ERR, GENESIS_PIPE,
                                "Unable to remove pipe: " + std::string(::strerror(errno)));
    }
}

std::unique_ptr<Action>
GenesisPipe::process_event(void)
{
    char buffer[256];
    ssize_t length;
...
void
GenesisPipe::event_processed(const std::string & event)
{
}
