...
int SourceScriptsCallback(const struct dirent *entry) {
  if (strlen(entry->d_name) > 3) {
    if (std::strcmp(entry->d_name + std::strlen(entry->d_name) - 3, ".sh") ==
        0) {
      return 1;
    }
  }
...
NetlinkRoute::NetlinkRoute() {
  ...
  URouteConfiguration =
      new Configuration(SYSCONFDIR "config", "netlink-route", defaultconfig);
      ...
      while (envvars.good()) {
        ...
        if (line != "") {
          ...
          eventsubscriptions.push_back(
              eventhandler(scriptfile, function, pcrepp::Pcre(match)));
        }
      }
    }
  } else {
    Log::get_instance().log(ERR, _NETLINK_ROUTE,
...
// NETLINK_ROUTE
//
void *NetlinkRoute::OpenSocket(int domain, int type, int protocol,
                               int multicastgroup) {
  ...
  if (bind(netlinksocket, (struct sockaddr *)&source_address,
  ...
  int on = 1;
  if (setsockopt(netlinksocket, SOL_SOCKET, SO_PASSCRED, &on, sizeof(on)) ==
      -1) {
...
void NetlinkRoute::serialize_rtm_newaddr(std::ostringstream &event,
                                         nlmsghdr *header) {
  ...
  event << ";PREFIX_LENGTH="
        << static_cast<int>(ifaddrmsg_payload->ifa_prefixlen);
...
void NetlinkRoute::serialize_rtm_deladdr(std::ostringstream &event,
                                         nlmsghdr *header) {
  struct ifaddrmsg *ifaddrmsg_payload;
  ...
  ifaddrmsg_payload = (struct ifaddrmsg *)NLMSG_DATA(header);
  serialize_ifa_family(event, ifaddrmsg_payload->ifa_family);
  event << ";G_PREFIX_LENGTH="
        << static_cast<int>(ifaddrmsg_payload->ifa_prefixlen);
  event << ";G_ADDRESS_FLAGS=";
  serialize_ifa_flags(event, ifaddrmsg_payload->ifa_flags);
  event << ";G_ADDRESS_SCOPE="
        << static_cast<int>(ifaddrmsg_payload->ifa_scope);
...
void NetlinkRoute::serialize_rtm_protocol(std::ostringstream &event,
                                          int protocol) {
  switch (protocol) {
  case RTPROT_UNSPEC:
    event << "unknown";
...
Action *NetlinkRoute::GetEvent() {
  ...
  for (struct nlmsghdr *netlink_header = (struct nlmsghdr *)netlink_buffer;
    ...
    case RTM_NEWROUTE:
      ...
      event_strings << ";G_ADDRESS_FAMILY="
                    << static_cast<int>(rtmsg_payload->rtm_family);
      event_strings << ";g_DESTINATION_LENGTH="
                    << static_cast<int>(rtmsg_payload->rtm_dst_len);
      event_strings << ";g_SOURCE_LENGTH="
                    << static_cast<int>(rtmsg_payload->rtm_src_len);
      event_strings << ";G_TOS_FILTER="
                    << static_cast<int>(rtmsg_payload->rtm_tos);
      event_strings << ";G_ROUTING_TABLE_ID="
                    << static_cast<int>(rtmsg_payload->rtm_table);
      serialize_rtm_table(event_strings, rtmsg_payload->rtm_table);
      event_strings << ";G_ROUTING_PROTOCOL="
                    << static_cast<int>(rtmsg_payload->rtm_protocol);
      serialize_rtm_protocol(event_strings, rtmsg_payload->rtm_protocol);
      event_strings << ";G_ROUTING_SCOPE="
                    << static_cast<int>(rtmsg_payload->rtm_scope);
      serialize_rtm_scope(event_strings, rtmsg_payload->rtm_scope);
      event_strings << ";G_ROUTING_TYPE="
                    << static_cast<int>(rtmsg_payload->rtm_type);
    ...
    case RTM_DELROUTE:
      ...
      event_strings << ";G_ROUTING_TYPE="
                    << static_cast<int>(rtmsg_payload->rtm_type);
      event_strings << ";G_ROUTING_FLAGS="
                    << static_cast<int>(rtmsg_payload->rtm_flags);
      break;
    ...
    case RTM_NEWNEIGH:
      ...
      event_strings << ";G_INTERFACE="
                    << static_cast<int>(ndmsg_payload->ndm_ifindex);
    ...
    case RTM_DELNEIGH:
      ...
      break;
    default:
      event_strings << ";unknown_netlink_event_received="
                    << netlink_header->nlmsg_type;
      break;
    }
  }
  ...
  for (auto iter : eventsubscriptions) {
    if (iter.match.search(event)) {
      if (URouteConfiguration->get_option("log_matched_events") == "yes") {
        Log::get_instance().log(DEBUG, _NETLINK_ROUTE,
                                "Matched route event: " + event);
        return new BashAction("run-function", iter.filename, iter.function,
                              tokenise(event, ";"));
      }
    }
  }
...
int NetlinkRoute::get_fd() { return netlinksocket; }

Action *NetlinkRoute::new_event(std::string event) { return NULL; }
